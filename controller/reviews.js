...
exports.getReviews = asyncHandler( async (req, res, next) => {
    if (req.params.bootcampsId) {
        const reviews = await Review.find({ bootcamp: req.params.bootcampsId })
...
exports.addReview = asyncHandler( async (req, res, next) => {
    ...
    if (!bootcamp) {
        return next(new ErrorResponse(`No bootcamp with the id of ${req.params.bootcampsId}`, 404))
    }
...
exports.updateReview = asyncHandler( async (req, res, next) => {
    ...
    if (review.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorResponse(`Not authorize to update review`, 400))
    }
